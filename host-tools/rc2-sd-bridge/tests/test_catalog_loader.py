"""Real mksh L5 lifecycle tests using synthetic payloads and command responses.
...
SOURCE = common.SOURCE.with_name("rc2_policy_catalog_loader.sh")
SID = common.SID


def native_text(sid=SID, pid=4242, uid=10123, mode="value", **changes):
    base = f"schema=finduas-policy-set/v1 phase={{phase}} sid={sid} pid={pid} uid={uid} gid={uid} abi_bits=32"
    values = dict(ready=1, stage=0, exception=0, cloud_query_count=0, product_query_count=1,
                  mmkv_decode_count=1, namespace_present=1, mmkv_present=1, cloud_present=-1,
                  product_present=1, product_type=139, receiver_type=-1, receiver_index=-1,
                  json_rc=0, entry_count=3, duplicate_count=0, candidate_count=-1, match_count=-1,
                  default_match=-1, product_blocked_count=0, jni_rc=0, env_rc=0, guard_rc=0,
    ...
    export = f"schema=finduas-policy-set/v1 phase=export sid={sid} pid={pid} export_rc=0 export_bytes=200 row_count=3 nonempty_count=2 distinct_count=1 default_present=1 default_nonempty=1\n"
...
class Fixture(common.Fixture):
    def __init__(self):
        super().__init__()
        self.target = self.root / "data/app/finduas_A060_policy_set.so"
        source = self.base / "FindUAS_POLICY_SET.so"
        self.payload.rename(source); self.payload = source
        self.receipt = self.probe / "A060_copy.receipt"
        self.marker = self.probe / "A060_attach.attempted"
        mock_source = common.MOCK_SOURCE
        for old, new in (("L1_TEST_ROOT", "L5_TEST_ROOT"),
                         ("finduas_A048_identity.so", "finduas_A060_policy_set.so"),
                         ("FindUAS_ARTTI_V2.so", "FindUAS_POLICY_SET.so"),
                         ("A048_copy.receipt", "A060_copy.receipt"),
                         ("A048_attach.attempted", "A060_attach.attempted"),
                         ("FindUAS-ARTTI-Identity:I", "FindUAS-Policy-Set:I"),
                         ("FindUAS-Loader", "FindUAS-Catalog-Loader")):
            mock_source = mock_source.replace(old, new)
        (self.bin / "mock.py").write_text("#!" + sys.executable + "\n" + mock_source)
        text = SOURCE.read_text()
        text = re.sub(r"^L5_SHA=.*$", "L5_SHA=" + hashlib.sha256(common.PAYLOAD).hexdigest(), text, flags=re.M)
        text = re.sub(r"^L5_SIZE=.*$", "L5_SIZE=" + str(len(common.PAYLOAD)), text, flags=re.M)
        text = re.sub(r"^L5_APK_SHA=.*$", "L5_APK_SHA=" + hashlib.sha256(common.APK).hexdigest(), text, flags=re.M)
        ...
        self.script = self.base / "L5.sh"; self.script.write_text(text)
        self.env["L5_TEST_ROOT"] = str(self.root)
    ...
    def run(self, operation, sid=SID):
        ...
        expected = f"schema=finduas-rc2-policy-catalog-loader/v1\nsid={sid}\noperation={operation}\n"
        if not result.stdout.startswith(expected) or not result.stdout.endswith("report_end=true\n"):
            raise AssertionError(result.stdout)
        return result.returncode, result.stdout


@unittest.skipUnless(common.REQUESTED_MKSH, "Set MKSH to run L5 host integration")
class CatalogLoaderTests(unittest.TestCase):
    ...
    def test_observed_relation_read_once_and_cleanup(self):
        f = self.fixture()
        self.assertEqual(f.run("CATALOG_BASELINE")[0], 0)
        self.assertFalse(f.target.exists()); self.assertEqual(f.count("attach"), 0)
        rc, output = f.run("CATALOG_READ")
        self.assertEqual(rc, 0, output)
        self.assertIn("policy_catalog_state=OBSERVED\n", output)
        self.assertIn("native_match_count=-1\n", output)
        self.assertIn("native_cloud_query_count=0\n", output)
        for name in ("product_query_count", "mmkv_decode_count"):
    ...
    def test_no_native_result_preserves_file_then_late_unknown_cleanup(self):
        ...
        self.assertEqual(f.run("CATALOG_READ")[0], 75)
        self.assertTrue(f.target.exists()); self.assertEqual(f.count("attach"), 1)
        self.assertEqual(f.run("CATALOG_CLEANUP")[0], 75)
        f.publish_native(mode="mmkv_absent")
        self.assertEqual(f.run("CATALOG_CLEANUP")[0], 0)
    ...
    def test_changed_file_and_record_read_errors_preserve_candidate(self):
        for mode in ("inode_changed", "hash_changed", "copy_error"):
            with self.subTest(mode=mode):
                f = self.fixture(); f.configure(mode=mode)
                self.assertEqual(f.run("CATALOG_READ")[0], 74)
                self.assertEqual(f.run("CATALOG_CLEANUP")[0], 73)
                self.assertTrue(f.target.exists()); self.assertEqual(f.count("remove"), 0)
        f = self.fixture(); f.configure(mode="no_result")
        self.assertEqual(f.run("CATALOG_READ")[0], 75)
    ...
    def test_cross_session_new_boot_cleanup_does_not_read_again(self):
        ...
        self.assertEqual(f.run("CATALOG_CLEANUP", common.OTHER_SID)[0], 0)
        self.assertFalse(f.target.exists()); self.assertTrue(f.marker.exists())
        self.assertEqual(f.count("attach"), 1)
    ...
    def test_parser_rejects_bad_contract_without_synthetic_loader_handler(self):
        ...
        vectors = [dict(mode="malformed"), dict(mode="no_enter"), dict(mode="duplicate"),
                   ...
                   dict(cloud_query_count=1), dict(cloud_present=0), dict(candidate_count=0),
        ...
        for values in (dict(mode="mmkv_absent"), dict(mode="failure"),
