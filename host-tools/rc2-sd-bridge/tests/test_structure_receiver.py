"""B5 fixed policy dispatch through real mksh/Java, with no device access."""
import hashlib
...
B5 = common.SOURCE.with_name("rc2_sd_structure_bridge.sh")


@unittest.skipUnless(common.MKSH and common.JAVA and common.JAVAC, "set MKSH and provide a JDK")
class StructureReceiverTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def session(self, valid_hash=True, replace_helper=False):
        session = common.Session(self.classes, source=B5, script_name="B5.sh",
                                 start_marker="B5_START_REQUESTED", ttl=300)
        self.addCleanup(session.cleanup)
        helper = ("printf 'TEST_L4 argc=%s op=%s sid=%s\\n' \"$#\" \"$1\" \"$2\"\n"
                  "case \"$1\" in STRUCTURE_BASELINE) exit 0 ;; STRUCTURE_READ) exit 10 ;; "
                  "STRUCTURE_CLEANUP) exit 0 ;; *) exit 99 ;; esac\n").encode()
        (session.base / "L4.sh").write_bytes(helper)
        expected = hashlib.sha256(helper).hexdigest() if valid_hash else "0" * 64
        session.script.write_text(re.sub(r"B1_L4_EXPECTED_SHA=[^\n]+",
                                        "B1_L4_EXPECTED_SHA=" + expected, session.script.read_text()))
        if replace_helper: session.env["B1_TEST_REPLACE_F4"] = str(session.base / "L4.sh")
        session.launch()
        self.assertEqual((session.session / "session.receiver").read_text(),
                         "B1 RECEIVER " + common.SID + " B5 END\n")
        return session

    def test_fixed_policy_operations_and_old_canary_operation_rejected(self):
        session = self.session()
        for seq, op, result in ((1, "PING", 0), (2, "STRUCTURE_BASELINE", 0),
                                (3, "STRUCTURE_READ", 10), (4, "STRUCTURE_CLEANUP", 0),
                                (5, "CANARY_LOAD", 65), (6, "RID_READ", 65)):
            session.put(seq, op)
            rc, report = session.result(seq)
            self.assertEqual(rc, result)
            if op.startswith("STRUCTURE_"):
                self.assertIn("TEST_L4 argc=2 op=" + op + " sid=" + common.SID, report)
            elif op in ("CANARY_LOAD", "RID_READ"): self.assertNotIn("TEST_L4", report)
    ...
    def test_wrong_helper_hash_never_executes(self):
        ...
        session.put(1, "STRUCTURE_READ")
        rc, report = session.result(1)
        self.assertEqual(rc, 65); self.assertNotIn("TEST_L4", report)
    ...
    def test_verified_ram_snapshot_survives_file_replacement(self):
        ...
        session.put(1, "STRUCTURE_BASELINE")
        rc, report = session.result(1)
        self.assertEqual(rc, 0)
        self.assertIn("TEST_L4 argc=2 op=STRUCTURE_BASELINE", report)
        self.assertNotIn("UNVERIFIED_FILE_EXECUTED", report)
        self.assertIn("UNVERIFIED_FILE_EXECUTED", (session.base / "L4.sh").read_text())
        session.put(2, "STOP"); session.result(2); session.closed("STOP")
