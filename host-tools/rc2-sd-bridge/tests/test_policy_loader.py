"""Real mksh L3 lifecycle tests using synthetic payloads and command responses.
...
SOURCE = common.SOURCE.with_name("rc2_cloud_policy_loader.sh")
SID = common.SID


def native_text(sid=SID, pid=4242, uid=10123, mode="value", **changes):
    base = f"schema=finduas-cloud-cache/v1 phase={{phase}} sid={sid} pid={pid} uid={uid} gid={uid} abi_bits=32"
...
class Fixture(common.Fixture):
    def __init__(self):
        super().__init__()
        self.target = self.root / "data/app/finduas_A054_cloud_policy.so"
        source = self.base / "FindUAS_CLOUD_POLICY.so"
        self.payload.rename(source); self.payload = source
        self.receipt = self.probe / "A054_copy.receipt"
        self.marker = self.probe / "A054_attach.attempted"
        mock_source = common.MOCK_SOURCE
        for old, new in (("L1_TEST_ROOT", "L3_TEST_ROOT"),
                         ("finduas_A048_identity.so", "finduas_A054_cloud_policy.so"),
                         ("FindUAS_ARTTI_V2.so", "FindUAS_CLOUD_POLICY.so"),
                         ("A048_copy.receipt", "A054_copy.receipt"),
                         ("A048_attach.attempted", "A054_attach.attempted"),
                         ("FindUAS-ARTTI-Identity:I", "FindUAS-Cloud-Cache:I"),
                         ("FindUAS-Loader", "FindUAS-Policy-Loader")):
            mock_source = mock_source.replace(old, new)
        (self.bin / "mock.py").write_text("#!" + sys.executable + "\n" + mock_source)
        text = SOURCE.read_text()
        text = re.sub(r"^L3_SHA=.*$", "L3_SHA=" + hashlib.sha256(common.PAYLOAD).hexdigest(), text, flags=re.M)
        text = re.sub(r"^L3_SIZE=.*$", "L3_SIZE=" + str(len(common.PAYLOAD)), text, flags=re.M)
        text = re.sub(r"^L3_APK_SHA=.*$", "L3_APK_SHA=" + hashlib.sha256(common.APK).hexdigest(), text, flags=re.M)
        ...
        self.script = self.base / "L3.sh"; self.script.write_text(text)
        self.env["L3_TEST_ROOT"] = str(self.root)
    ...
    def run(self, operation, sid=SID):
        ...
        expected = f"schema=finduas-rc2-cloud-policy-loader/v1\nsid={sid}\noperation={operation}\n"
        if not result.stdout.startswith(expected) or not result.stdout.endswith("report_end=true\n"):
            raise AssertionError(result.stdout)
        return result.returncode, result.stdout


@unittest.skipUnless(common.REQUESTED_MKSH, "Set MKSH to run L3 host integration")
class PolicyLoaderTests(unittest.TestCase):
    ...
    def test_observed_relation_read_once_and_cleanup(self):
        f = self.fixture()
        self.assertEqual(f.run("POLICY_BASELINE")[0], 0)
        self.assertFalse(f.target.exists()); self.assertEqual(f.count("attach"), 0)
        rc, output = f.run("POLICY_READ")
        self.assertEqual(rc, 0, output)
        self.assertIn("policy_cache_state=OBSERVED\n", output)
        ...
        self.assertNotEqual(f.run("POLICY_READ")[0], 0)
    ...
    def test_absent_inputs_are_terminal_unknown_with_owned_file_cleanup(self):
        for mode in ("namespace_absent", "mmkv_absent", "cloud_absent", "product_absent"):
            with self.subTest(mode=mode):
                ...
                self.assertIn("policy_cache_state=UNKNOWN\n", output)
    ...
    def test_unpinned_and_preflight_identity_changes_never_dispatch(self):
        f = self.fixture()
        f.script.write_text(re.sub(r"^L3_SHA=.*$", "L3_SHA=UNSET", f.script.read_text(), flags=re.M))
        self.assertEqual(f.run("POLICY_READ")[0], 69)
    ...
    def test_no_native_result_preserves_file_then_late_unknown_cleanup(self):
        ...
        self.assertEqual(f.run("POLICY_READ")[0], 75)
        self.assertTrue(f.target.exists()); self.assertEqual(f.count("attach"), 1)
        self.assertEqual(f.run("POLICY_CLEANUP")[0], 75)
        f.publish_native(mode="mmkv_absent")
        self.assertEqual(f.run("POLICY_CLEANUP")[0], 0)
    ...
    def test_changed_file_and_record_read_errors_preserve_candidate(self):
        for mode in ("inode_changed", "hash_changed", "copy_error"):
            with self.subTest(mode=mode):
                f = self.fixture(); f.configure(mode=mode)
                self.assertEqual(f.run("POLICY_READ")[0], 74)
                self.assertEqual(f.run("POLICY_CLEANUP")[0], 73)
                self.assertTrue(f.target.exists()); self.assertEqual(f.count("remove"), 0)
        f = self.fixture(); f.configure(mode="no_result")
        self.assertEqual(f.run("POLICY_READ")[0], 75)
    ...
    def test_cross_session_new_boot_cleanup_does_not_read_again(self):
        ...
        self.assertEqual(f.run("POLICY_CLEANUP", common.OTHER_SID)[0], 0)
        self.assertFalse(f.target.exists()); self.assertTrue(f.marker.exists())
        self.assertEqual(f.count("attach"), 1)
