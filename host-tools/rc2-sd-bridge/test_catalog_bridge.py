...
class CatalogClientTest(unittest.TestCase):
    ...
    def catalog_ready(self):
        ...
        self.remote.files[f"{bridge.BASE}/{sid}/session.receiver"] = (
            f"B1 RECEIVER {sid} B6 END\n".encode())
        return sid

    def body(self, sid, operation, ready=True):
        return (f"schema=finduas-rc2-policy-catalog-loader/v1\nsid={sid}\noperation={operation}\n"
    ...
    def baseline(self, sid, seq="0001", rc=0, ready=True):
        self.client.submit("CATALOG_BASELINE")
        self.result(sid, seq, rc, self.body(sid, "CATALOG_BASELINE", ready))
        self.client.collect(seq)

    def test_b6_marker_is_required_and_other_lanes_cannot_substitute(self):
        sid = self.ready()
        receiver = f"{bridge.BASE}/{sid}/session.receiver"
        for marker in (None, f"B1 RECEIVER {sid} B2 END\n".encode(),
                       b"B1 RECEIVER 0123456789abcdef B6 END\n",
                       f"B1 RECEIVER {sid} B6 END\r\n".encode()):
            if marker is not None:
                self.remote.files[receiver] = marker
            for operation in bridge.CATALOG_OPS:
        ...
        self.remote.files[receiver] = f"B1 RECEIVER {sid} B6 END\n".encode()
        with self.assertRaises(bridge.BridgeError):
            self.client.submit("RID_BASELINE")
        self.assertEqual("TASK_SUBMITTED seq=0001", self.client.submit("CATALOG_BASELINE"))

    def test_read_needs_latest_collected_valid_successful_baseline(self):
        sid = self.catalog_ready()
        with self.assertRaisesRegex(bridge.BridgeError, "SUCCESSFUL_CATALOG_BASELINE_REQUIRED"):
            self.client.submit("CATALOG_READ")
        self.baseline(sid)
        self.client.submit("CATALOG_BASELINE")
        self.result(sid, "0002", 0, self.body(sid, "CATALOG_BASELINE"))
        with self.assertRaisesRegex(bridge.BridgeError, "SUCCESSFUL_CATALOG_BASELINE_REQUIRED"):
            self.client.submit("CATALOG_READ")
        ...
        self.assertEqual("TASK_SUBMITTED seq=0006", self.client.submit("CATALOG_READ"))
    ...
    def test_uncertain_transfer_reuses_job_and_terminal_read_is_not_repeated(self):
        ...
        with self.assertRaisesRegex(bridge.BridgeError, "PENDING_DIFFERENT_OPERATION"):
            self.client.submit("CATALOG_CLEANUP")
        self.assertEqual("TASK_SUBMITTED seq=0002", self.client.submit("CATALOG_READ"))
        self.assertEqual(saved, self.client.task_path(sid, "0002", ".job").read_bytes())
        self.result(sid, "0002", 10, self.body(sid, "CATALOG_READ"))
        self.client.collect("0002")
        with self.assertRaisesRegex(bridge.BridgeError, "CATALOG_READ_ALREADY_CREATED"):
            self.client.submit("CATALOG_READ")
        self.assertEqual("TASK_SUBMITTED seq=0003", self.client.submit("CATALOG_CLEANUP"))

    def test_incomplete_read_can_be_cleaned_up_but_not_replayed(self):
        sid = self.catalog_ready()
        self.baseline(sid)
        self.client.submit("CATALOG_READ")
        partial = self.body(sid, "CATALOG_READ").removesuffix(b"report_end=true\n")
        self.result(sid, "0002", 124, partial)
        self.client.collect("0002")
        summary = json.loads(self.client.task_path(sid, "0002", ".collected.json").read_bytes())
        self.assertEqual("incomplete_envelope", summary["catalog_validation"])
        self.assertFalse(summary["catalog_preflight_ready"])
        with self.assertRaisesRegex(bridge.BridgeError, "CATALOG_READ_ALREADY_CREATED"):
            self.client.submit("CATALOG_READ")
        self.assertEqual("TASK_SUBMITTED seq=0003", self.client.submit("CATALOG_CLEANUP"))
    ...
    def test_inner_command_text_cannot_supply_preflight_ready(self):
        sid = self.catalog_ready()
        self.client.submit("CATALOG_BASELINE")
        body = self.body(sid, "CATALOG_BASELINE").replace(b"preflight_ready=true\n",
            b"BEGIN fixture\npreflight_ready=true\ncommand.fixture.rc=0\nEND fixture\n")
        self.result(sid, body=body)
        self.client.collect("0001")
        with self.assertRaisesRegex(bridge.BridgeError, "SUCCESSFUL_CATALOG_BASELINE_REQUIRED"):
            self.client.submit("CATALOG_READ")
        self.assertEqual("TASK_SUBMITTED seq=0002", self.client.submit("CATALOG_CLEANUP"))

    def test_zero_rc_foreign_envelope_is_terminal_not_a_successful_baseline(self):
        sid = self.catalog_ready()
        self.client.submit("CATALOG_BASELINE")
        body = self.body(sid, "CATALOG_BASELINE").replace(
            b"schema=finduas-rc2-policy-catalog-loader/v1", b"schema=finduas-rc2-canary-loader/v1")
        self.result(sid, body=body)
        self.client.collect("0001")
        summary = json.loads(self.client.task_path(sid, "0001", ".collected.json").read_bytes())
        self.assertEqual("invalid_envelope", summary["catalog_validation"])
        with self.assertRaisesRegex(bridge.BridgeError, "SUCCESSFUL_CATALOG_BASELINE_REQUIRED"):
            self.client.submit("CATALOG_READ")
    ...
    def test_stage_only_transports_three_fixed_files(self):
        ...
        for index in range(3):
            ...
            inputs.append(path)
        self.assertEqual("SCRIPTS_STAGED_VERIFIED", self.client.stage_catalog(*inputs))
        self.assertEqual(["Download/B6.sh", "Download/L5.sh", "Download/FindUAS_POLICY_SET.so"],
