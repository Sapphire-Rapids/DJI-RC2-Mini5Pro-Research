...
PATTERNS = {
    ...
    "JOB": rf"B1 JOB (?P<sid>{SID}) (?P<seq>{SEQ}) (?P<op>PING|SNAPSHOT|STOP) END\n",
...
class Client:
    ...
    def stage(self, b1: Path, f4: Path) -> str:
        for basename, source in (("B1.sh", b1), ("F4.sh", f4)):
...
def main(argv=None) -> int:
    ...
    submit.add_argument("operation", choices=("PING", "SNAPSHOT", "STOP"))
