...
| C-138 | `HYPOTHESIS` | A fixed RC 2 Binder client can decide Mini 5 Pro admission with one F7/F8 probe and, only after a baseline, perform one F9/readback/restore loop | APK staging is not a live result; installation, execution, command replies, and RF effects remain unobserved |
