...
| C-032 | `RETRACTED` | The adjacent-version-only explanation is superseded by exact signed-v07 package evidence | C-237 later records production/empty boot-property reads; mounted adbd, taken branch and userspace-copy execution remain pending |
...
| C-175 | `STATIC` | Exact v07 `handle_packet(CNXN)` contains the `mp_state=production && dbg_cnt<1` early return before AUTH; runtime executable is `/apex/com.android.adbd/bin/adbd` | C-237 records production/empty property reads; mounted adbd and taken branch remain pending |
| C-176 | `STATIC` | Exact v07 `dpad_fuli.apk` is byte-identical to the audited package and contains the system-shared-UID shell-command page using `Runtime.exec` | Later C-235/C-237 establish live package hash and reported identity; no command result or loader execution |
...
| C-234 | `OBSERVED` | Exact A-038 APK staged in RC 2 removable-SD Download and fully read back with matching size/hash | APK-transfer observation; installation/run and report receipt are subsequently recorded in C-235 |
| C-235 | `OBSERVED` | A-038 live report received: Fly 1.19.4 / code 3113157, ARM32 packaged/runtime baseline and readable SDK JNI; ART build-ID check remains INCOMPLETE | Identity fields remain usable; no loader or FlySafe/RID result |
| C-236 | `STATIC` | A-039 adds ARM32 ART reading, disabled-component visibility and fixed installed-sample export; 94 JVM + 8 auditor tests, 37 rejected mutations, identical clean builds, zero permissions, v2/zipalign | Artifact checks only; report-completion semantics unchanged |
| C-237 | `OBSERVED` | A-039 installed/run; COMPLETE report received with ARM32 ART build ID, matching framework/services and Fuli identity, disabled components, ro.debuggable=1, production and SELinux enforcing=false | Empty dbg_cnt is not zero; COMPLETE is inspection evidence, not loader execution |
| C-238 | `OBSERVED` | Exact Fly APK and three SDK libraries received and independently validated; APK version/ABI/signer and JNI report hash match; local reader corrected for libmtp progress payload+16 | Initial transfer returned error; completeness rests on independent payload verification, not a successful transport return |
...
| C-260 | `STATIC` | F3 pairs strict single-main-process AMS reads with proc metadata, separate stability fields and full proc mount line; 14 parser/capture vectors and 18 host body fixtures passed | Host-shell checks; C-262 later identifies the Android mksh heredoc failure |
| C-261 | `OBSERVED` | F3 SD staging/full readback matches 10611 bytes and registered SHA; F2 archived with matching full before/after readbacks | No deletion; subsequent F3 execution/report in C-262 |
...
| C-263 | `STATIC` | F4 replaces the heredoc with a printf pipe; 18 Android mksh body fixtures and 12 temp-policy comparisons pass | Fixed reads and SD report boundary unchanged; live F4 follows in C-267 |
| C-264 | `STATIC` | B1 finite fixed diagnostic jobs, immutable input/result records and pinned RAM-executed F4; 11 real mksh/Java cases, 19 client tests and C transport checks pass | Live startup/round trip follows in C-266; session records stay private |
