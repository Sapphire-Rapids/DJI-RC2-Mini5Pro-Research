...
| C-151 | `STATIC` | A-025 final APK identity/audit passed 42 tests, lint with 0 errors/9 warnings, clean assemble, byte-identical rebuild, v2-signature/alignment checks, zero permissions, no native/network/socket/shell path, redacted reply handling, and no admitted `11/12`; the old `11/1C` button is removed | A local delivery copy exists, but none has been copied to RC 2 removable storage, installed, or run; no Binder, inventory, entitlement, device-state, or RF result exists, and the binary remains outside this repository |
