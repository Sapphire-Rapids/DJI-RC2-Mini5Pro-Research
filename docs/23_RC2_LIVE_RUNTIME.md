...
（操作者确认固件 `01.00.0600`）。本页汇总 C-235--C-315；行动顺序见
...
| 全组策略读取 | A060完成41行采集、全部签名核验和独立清理；解析到13/26字节正文 | C-310--C-314 |
...
C-308记录程序准备；SD staging、新会话及后续实机结果分别登记。

## 下一步

A060采集、验签与独立清理均完成。现有相关官方枚举未匹配候选编号；下一步追踪飞机端
`00/DD`中逐13字节推进、按段偏移+1分派的消费者，重点核对双段配置。字段解释保留在H-33。
当前无需新的实机操作。

## 同步状态

按操作者的一次性要求，既有六个提交已推送至 GitHub main，终点为 `2f31394`。
本轮实际采集和结构分析继续在本地记录，未再次推送。
...
操作者所需的`Download/B6.sh`准确启动命令已私下提供。基线、采集及独立清理按新会话分别记录。

## B6启动与全组采集基线（C-310）

`OBSERVED`：操作者启动B6后，完整`CATALOG_BASELINE`任务0001报告已收到并通过独立解析。
返回码为0，23项检查全部为true，`preflight_ready=true`；检查覆盖身份、网络/更新设置、
APK与源文件、目录/日志控制及AMS稳定性。`attach_dispatch_count=0`，未创建内部测试文件，
`native_result_observed=false`。

启动和基线已完成，当前等待全组策略采集报告。此处不包含`CATALOG_READ`的执行或内容结果。


## A060整组策略实机结果（C-311/C-312）

`OBSERVED`：B6基线通过后，`CATALOG_READ`重复23项预检，加载固定26624-byte A060一次。
12615-byte报告的原生enter/export/result完整匹配：MMKV解码1次、ProductType缓存查询1次、
CloudControlData查询0次。产品为139，41行全部非空、36个不同hex字符串，没有重复country
或被产品过滤的行；首个DEFAULT存在且非空。JNI、环境、guard、JSON及Dispose结果均为0。
Fly PID/UID/APK保持一致，经过身份/hash校验的测试文件已删除。

导出的10907-byte JSON完整回传，严格schema与所有长度/计数均与原生日志一致。
独立`CATALOG_CLEANUP`返回0、`cleanup_already_absent=true`、attach0；A060永久copy/attempt
回执与工件、会话、boot、目标及文件身份匹配。此前A057原件/回执保持原样。
本轮未发送STOP，保留B6供当前分析期间使用；原始配置、country行、签名与设备身份仍不入库。


## 完整策略集的正文结构（C-313/C-314）

`CORROBORATED`：41行非空记录形成36个完整签名包，全部用A057的同一P-256验证点通过
SHA-256验签。去重后只有25个不同的头部+正文消息，11组相同消息对应不同的原始签名。
完整正文则只有6种：40行是13字节（5种），另1行是26字节。A057原matched与DEFAULT
各仍精确匹配当前集合中的1行。

按13字节切分可得到42段、6种字节模式。26字节正文的前一段等于常见单段正文，后一段
沿用同样的数值对齐。头部+6在两种长度下始终为1，不能用作这些候选段的数量。

| 段内位置 | 候选读法 | 全组取值 |
| --- | --- | --- |
| +0 | u8 | 0出现1次、1出现41次 |
| +1 | LE16 | 2、3、4、5 |
| +3 / +4 | 两个u8，或原LE16/组合mask解释 | (0,0)、(0,1)、(0,2)、(2,0) |
| +5 | LE32 | 16、20 |
| +9 | LE32 | 3、14、16 |

重复记录结构、类型编号和启停解释仍集中记录在H-33；下一判别是把候选编号与官方枚举
关联，并找到读取+0/+3/+4的实际分支。未按DEFAULT内容进行切换。

独立解析器增加`body.repeated_layout_candidate`，仅为非空13字节倍数给出带假设标签的
分段和数值，不使用头部+6推段数。原13字节输出和验证保持不变；23项envelope、15项签名、
14项通用结构测试均通过，另有12项私有全组分析和7项回执检查测试。


## 候选编号与现有枚举比较（C-315）

`NEGATIVE`：确切Fly1.19.4的RIDInfoOpt/RIDResultState属于注册操作/导入结果；MSDK5.18的
RemoteIdWorkingState属于工作及错误状态。RidUnlockType和字符串型AreaStrategy也未匹配
私有配置分组，未找到能逐值解释2/3/4/5的记录类型定义。下一搜索特征是实际消费者中的
13字节步长和段偏移+1分派，尤其是26字节项的第二段；不从同值状态枚举给字段定名。

本轮物理路线为RC2可移除SD上的固定B6/L5，经开发助手启动，再由守护MTP工具提交/回收。
预检/读取/独立清理预算分别120/210/30秒；native日志严格绑定当前SID和目标PID/UID，
有序匹配enter/export/result。读取1次，追加独立清理1次；末次文件不存在、回执匹配。
本轮没有策略状态变更或独立RF采样。
