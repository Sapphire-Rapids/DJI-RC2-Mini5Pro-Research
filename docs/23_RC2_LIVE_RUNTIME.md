...
（操作者确认固件 `01.00.0600`）。本页汇总 C-235--C-305；行动顺序见
...
本轮实机采集和文件清理已完成，当前无需额外实机操作。继续追飞机端`00/DD`接收处理器，
将13字节正文的首字节分支与实际字段名对应。整包offset20（正文offset0）的1/0是首要
启停候选；其他候选字段同时保留，避免把格式版本/区域策略误命名为开关。
