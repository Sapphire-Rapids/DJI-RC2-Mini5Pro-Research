...
（操作者确认固件 `01.00.0600`）。本页汇总 C-235--C-262；行动顺序见
...
| 当前操作 | F3 已读回并定位 mksh 临时文件问题；修正诊断并实现一次启动的 SD 任务接收器 | C-262 |
...
2. F3 报告已收到，修复 heredoc 的 mksh 兼容性问题，不再重跑原版 F3。
3. 按操作者新请求，实现有限会话的 SD 任务接收器和主机客户端，减少逐条手工输入；
   完成验证后再提供一次启动命令。
