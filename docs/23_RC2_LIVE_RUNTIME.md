...
（操作者确认固件 `01.00.0600`）。本页汇总 C-235--C-259；行动顺序见
...
| 当前操作 | 准备 F3 同次报告：AMS 前后 PID、目标进程文件与完整 proc 挂载行 | C-259 |
...
2. F2、AMS 与直接路径结果已收到，当前准备 F3 同次报告；无需重复这些手工命令。
3. F3 完成检查与暂存后，再给出一条运行命令。
