...
- `OBSERVED`：操作者启动B6，23项基线检查全部通过（C-310）。
- `OBSERVED`：A060一次解码/一次ProductType读取，取得41条非空记录、36不同签名包；
  PID/UID/APK稳定、临时文件删除（C-311）。
- `OBSERVED`：10907-byte报告完整收到，独立清理和永久回执验证通过；B6未发送STOP（C-312）。
- `CORROBORATED`：36包全部验签；独立比较得到25种签名消息、6种完整正文及13/26字节
  两种正文长度，A057两份旧样本仍各匹配1行（C-313）。
- `STATIC`：解析器补充带假设标签的重复13字节段与分字节解释，52项公开测试通过（C-314）。
  配置/签名原文保持私有，本轮结果和代码继续同步本地。

- `NEGATIVE`：相关Fly/MSDK枚举未匹配候选2/3/4/5分组；后续转向逐13字节推进的实际
  接收消费者，未增加设备请求（C-315）。
