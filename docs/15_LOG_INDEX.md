...
- Android mksh 构建、F4 的 18 个完整场景、12 个临时目录对照与 MTP 回读记录支持
  C-263；构建产物、主机测试路径和原始输出保留本地。
- B1 的 Java/mksh 集成日志、主机 transport/client 检查、MTP 错误及后续同会话准备记录
  支持 C-264/C-265；SD 卷、SID、任务输出和原始 USB 日志均排除。
- B1 READY、三项不可变任务/回执/报告及 F4 严格解析索引支持 C-266/C-267；
  报告、PID、卷号、会话 ID、采样时间和本地工作目录继续保留私有。
...
- v07 AMS/ART TI 的定向方法与生命周期核对支持 C-268；厂商代码与反汇编留在本地。
- A-048 源码检查、32 项测试、sanitizer、故障变体、ELF 符号及相同构建记录支持 C-269。
- A-048 SD staging/readback 与 B1 STOP/closed 回执支持 C-270；原生库、完整日志和会话标识排除。
- L1/B2 的真实 mksh/Java 场景、29 项 client 测试、原生日志解析复核支持 C-271；
  暂存、旧 active 归档、完整回读和新会话记录支持 C-272。硬件标识及原始数据继续排除。
- B2 的四项任务、L1 严格解析、两次相同原生日志快照、copy/attempt 原件和独立核验
