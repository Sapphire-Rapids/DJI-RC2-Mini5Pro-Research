...
published_apk="$project_dir/dist/FindUAS-RID-Bridge-Probe-0.12.0-live32-samples.apk"
task_tmp="$(mktemp -d "${TMPDIR:-/tmp}/finduas-rid-v012-repro.XXXXXX")"

cleanup() {
    case "$task_tmp" in
        "${TMPDIR:-/tmp}"/finduas-rid-v012-repro.*) rm -rf -- "$task_tmp" ;;
