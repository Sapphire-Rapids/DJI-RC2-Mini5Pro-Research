...
实现并验证一个可控、可读回、可恢复的 Mini 5 Pro RID 开关。直接在仓库当前 `main` 上继续，
...
最后由用户起桨、标准 RID 接收器做 RF A-B-A；RID/FlySafe 写入不因类别被禁止，但必须满足
same-item baseline、路线证明、readback 和 restore 门禁；不得伪造、导出或重放 license/账号材料。

禁止解锁 Bootloader、OEM Unlock、bootloader reboot、修改或刷写 boot/vendor_boot/vbmeta/TEE/
QFPROM/eFuse、Magisk，也禁止会触发 TEE/eFuse tamper 状态或让 DJI Fly 自动更新进入不可恢复
启动路径的操作（C-212）。厂商 APK/固件/DEX/反编译源码、raw log/capture、设备/账号/license 标识不得
