#ifndef FINDUAS_EID_ROUTE_V23_ROUTE_RESOLVER_H
#define FINDUAS_EID_ROUTE_V23_ROUTE_RESOLVER_H
...
enum FinduasRouteStatus {
    ...
    FINDUAS_ROUTE_STATUS_RUNTIME_IDENTITY_FAILED = 16,
};
...
typedef struct FinduasRouteDiagnostic {
    ...
    uint32_t identity_error;
    uint32_t identity_module_id;
    uint32_t identity_stage;
    int32_t identity_errno;
    uint64_t identity_hashed_bytes;
    uint32_t identity_relevant_vma_count;
    uint32_t identity_opened_fd_count;
    uint32_t identity_closed_fd_count;
    uint32_t identity_verified_module_count;
