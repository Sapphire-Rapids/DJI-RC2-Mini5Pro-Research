...
#error "FindUAS EID route resolver V2.3 must be built for AArch64"
#endif

#define FINDUAS_LOG_TAG "FindUAS-EID-Route-V23"
...
static void emit_numeric_result(const FinduasAgentResult *result) {
    __android_log_print(
        ANDROID_LOG_INFO,
        FINDUAS_LOG_TAG,
        "FINDUAS_EID_ROUTE_V23 error=%u route=%u loaded=%d on=%d gate=%d loaders=%d "
        "module_error=%u modules=%u handles=%u symbols=%u mediator=%u exception_gate=%u "
        "identity=%u identity_module=%u identity_stage=%u identity_errno=%d hashed=%llu "
        "identity_vmas=%u opened_fds=%u closed_fds=%u identity_modules=%u "
        ...
        (unsigned int)result->route.identity_error,
        (unsigned int)result->route.identity_module_id,
        (unsigned int)result->route.identity_stage,
        (int)result->route.identity_errno,
        (unsigned long long)result->route.identity_hashed_bytes,
        (unsigned int)result->route.identity_relevant_vma_count,
        (unsigned int)result->route.identity_opened_fd_count,
        (unsigned int)result->route.identity_closed_fd_count,
        (unsigned int)result->route.identity_verified_module_count,
