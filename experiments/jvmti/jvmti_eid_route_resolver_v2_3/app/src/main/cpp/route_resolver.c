...
enum FinduasRouteStatus finduas_route_resolver_run(FinduasRouteDiagnostic *diagnostic) {
    ...
    FinduasIdentityModuleSet identity_modules;
    memset(&identity, 0, sizeof(identity));
    memset(&identity_modules, 0, sizeof(identity_modules));
    identity_modules.module_count = modules.validated_module_count;
    identity_modules.noload_handle_count = modules.opened_handle_count;
    for (uint32_t index = 0u; index < FINDUAS_MODULE_COUNT; ++index) {
        identity_modules.modules[index].base = modules.modules[index].base;
        identity_modules.modules[index].path = modules.modules[index].path;
        identity_modules.modules[index].path_length = modules.modules[index].path_length;
    }
    const enum FinduasIdentityError identity_error =
        finduas_runtime_identity_verify(&identity_modules, &identity);
    ...
    module_error = finduas_modules_admit_after_identity_recheck(&modules);
    diagnostic->module_error = (uint32_t)module_error;
    if (module_error != FINDUAS_MODULE_ERROR_NONE) {
        diagnostic->identity_error = FINDUAS_IDENTITY_LINKER_EPOCH_CHANGED;
        diagnostic->identity_stage = 13u;
        diagnostic->status = FINDUAS_ROUTE_STATUS_RUNTIME_IDENTITY_FAILED;
        finduas_modules_close(&modules);
        return diagnostic->status;
    }
