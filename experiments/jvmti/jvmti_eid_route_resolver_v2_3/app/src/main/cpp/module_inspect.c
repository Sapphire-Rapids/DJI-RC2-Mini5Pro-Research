...
#define FINDUAS_IDENTITY_ADMISSION UINT32_C(0x4944454e)
...
enum FinduasModuleError finduas_modules_admit_after_identity_recheck(
    ...
    set->identity_admission = 0u;
    const enum FinduasModuleError error = finduas_modules_recheck(set);
    if (error != FINDUAS_MODULE_ERROR_NONE) {
        return error;
    }
    set->identity_admission = FINDUAS_IDENTITY_ADMISSION;
    return FINDUAS_MODULE_ERROR_NONE;
}
...
enum FinduasModuleError finduas_modules_finalize_verified(FinduasModuleSet *set) {
    ...
    for (int module_index = 0; module_index < FINDUAS_MODULE_COUNT; ++module_index) {
        ...
        const FinduasModuleProfile *profile = &kFinduasModuleProfiles[module_index];
        if (module->base == 0u ||
            module->base > UINTPTR_MAX - sizeof(FinduasElf64Ehdr) ||
            module->runtime_phdr !=
                (const void *)(module->base + sizeof(FinduasElf64Ehdr)) ||
            module->runtime_phnum != profile->exact_phnum ||
            memcmp(
                (const void *)module->base,
                profile->elf_header,
                FINDUAS_ELF_HEADER_SIZE) != 0 ||
            memcmp(
                module->runtime_phdr,
                profile->phdrs,
                sizeof(profile->phdrs)) != 0) {
            return FINDUAS_MODULE_ERROR_SEGMENTS;
        }
