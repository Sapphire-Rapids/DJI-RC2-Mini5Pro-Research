...
static int file_headers_match(
    ...
    const int matched = pread_exact(fd, file_header, sizeof(file_header), 0u) &&
        pread_exact(
            fd,
            file_phdrs,
            sizeof(file_phdrs),
...
static enum FinduasIdentityError hash_and_compare_file(
    int fd,
    const FinduasIdentityModule *module,
    const FinduasModuleProfile *profile,
...
enum FinduasIdentityError finduas_runtime_identity_verify(
    ...
    if (modules == NULL || modules->module_count != FINDUAS_MODULE_COUNT ||
        modules->noload_handle_count != FINDUAS_MODULE_COUNT ||
    ...
    for (uint32_t index = 0u; index < FINDUAS_MODULE_COUNT; ++index) {
        const FinduasIdentityModule *module = &modules->modules[index];
        ...
        if (fstat(fd, &pre_status) != 0 ||
            !finduas_capture_file_state(&pre_status, &states[index]) ||
        ...
        if (module_error == FINDUAS_IDENTITY_OK) {
            stage = 7u;
            if (!file_headers_match(profile, fd)) {
        ...
        if (module_error == FINDUAS_IDENTITY_OK) {
            ...
            if (fstat(fd, &post_status) != 0 ||
                !finduas_capture_file_state(&post_status, &post_state) ||
                !finduas_file_states_equal(&states[index], &post_state)) {
    ...
    if (diagnostic->opened_fd_count != diagnostic->closed_fd_count ||
        ...
        set_error(diagnostic, FINDUAS_IDENTITY_CLOSE_FAILED, 0u, 12u);
        goto cleanup;
    }
