...
enum MapsMutation {
    ...
    MAPS_DELETED_SOURCE,
    MAPS_ALL_WRITABLE,
    MAPS_FIRST_NONWRITABLE_PAGE_WRITABLE,
    MAPS_HIGH_NONWRITABLE_WRITABLE,
    MAPS_ZERO_DEVICE,
};
...
static size_t build_maps_fixture(
    ...
    for (size_t index = 0u; index < profile->exact_phnum; ++index) {
        ...
        uint32_t major_value = mutation == MAPS_ZERO_DEVICE
            ? 0u
            : (mutation == MAPS_WRONG_DEVICE && load_number == 0u ? 0xfeu : 0xfdu);
        const uint32_t minor_value = mutation == MAPS_ZERO_DEVICE ? 0u : 1u;
        ...
        const char writable =
            mutation == MAPS_ALL_WRITABLE ||
                    (mutation == MAPS_HIGH_NONWRITABLE_WRITABLE && load_number == 3u) ||
        ...
        if (mutation == MAPS_SPLIT_FIRST && load_number == 0u) {
            ...
            used = append_map_line(
                ...
                offset, major_value, minor_value, inode, path);
            used = append_map_line(
                output, used, capacity, split, end, readable, writable, executable, sharing,
                offset + 2u * FINDUAS_RUNTIME_PAGE_SIZE,
                major_value, minor_value, inode, path);
        } else if (mutation == MAPS_FIRST_NONWRITABLE_PAGE_WRITABLE && load_number == 0u) {
            const uintptr_t split = start + FINDUAS_RUNTIME_PAGE_SIZE;
            used = append_map_line(
                output, used, capacity, start, split, readable, 'w', executable, sharing,
                offset, major_value, minor_value, inode, path);
            used = append_map_line(
                output, used, capacity, split, end, readable, writable, executable, sharing,
                offset + FINDUAS_RUNTIME_PAGE_SIZE,
...
static enum FinduasIdentityError parse_maps_fixture(
    ...
    const uint32_t expected_major = mutation == MAPS_ZERO_DEVICE ? 0u : 0xfdu;
    const uint32_t expected_minor = mutation == MAPS_ZERO_DEVICE ? 0u : 1u;
...
static void test_maps_and_paths(void) {
    ...
    assert(parse_maps_fixture(MAPS_ALL_WRITABLE, &split, 0) ==
           FINDUAS_IDENTITY_WRITABLE_ORIGINAL_NONWRITABLE_LOAD);
    assert(parse_maps_fixture(MAPS_FIRST_NONWRITABLE_PAGE_WRITABLE, &split, 0) ==
           FINDUAS_IDENTITY_WRITABLE_ORIGINAL_NONWRITABLE_LOAD);
    assert(parse_maps_fixture(MAPS_HIGH_NONWRITABLE_WRITABLE, &split, 0) ==
           FINDUAS_IDENTITY_WRITABLE_ORIGINAL_NONWRITABLE_LOAD);
    assert(parse_maps_fixture(MAPS_ZERO_DEVICE, &split, 0) ==
    ...
    assert(finduas_snapshot_contains_readable_nonwritable_range(
               &exact,
               exact.entries[0].start,
               FINDUAS_RUNTIME_PAGE_SIZE) == 1);
    assert(finduas_snapshot_contains_readable_range(
...
static void test_nonwritable_memory_compare(void) {
    ...
    assert(finduas_identity_compare_nonwritable_chunk(
               ...
               file_bytes,
               sizeof(file_bytes)) == 1);
    snapshot.entries[0].writable = 1u;
    assert(finduas_identity_compare_nonwritable_chunk(
               (uintptr_t)memory_bytes,
               profile,
               &snapshot,
               ...
               file_bytes,
               sizeof(file_bytes)) == 0);
    snapshot.entries[0].writable = 0u;
    memory_bytes[31] ^= 1u;
    assert(finduas_identity_compare_nonwritable_chunk(
               (uintptr_t)memory_bytes,
               profile,
...
static void test_file_state_zero_device_gate(void) {
    struct stat status;
    FinduasFileState state;
    memset(&status, 0, sizeof(status));
    memset(&state, 0, sizeof(state));
    status.st_dev = (dev_t)1;
    status.st_ino = (ino_t)2;
    status.st_size = (off_t)3;
    status.st_nlink = (nlink_t)1;
    status.st_mode = S_IFREG | S_IRUSR;
    assert(finduas_capture_file_state(&status, &state) == 1);
    status.st_dev = (dev_t)0;
    assert(finduas_capture_file_state(&status, &state) == 0);
}
...
int main(void) {
    ...
    test_file_state_zero_device_gate();
    return 0;
}
