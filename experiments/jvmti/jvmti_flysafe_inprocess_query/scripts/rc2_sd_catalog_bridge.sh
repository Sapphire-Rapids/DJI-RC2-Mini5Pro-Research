#!/system/bin/sh
# B6: finite B1-protocol mailbox with one fixed RID policy catalog export.
PATH=/system/bin
LC_ALL=C
export PATH LC_ALL
set -f
...
B1_L5_EXPECTED_SHA=a1e509cd10b55594151e3a5b9694247fab4a43b1850cf9602fccb23eac264060
...
fail_start() { printf 'B6_ERROR code=%s\n' "$1"; exit 64; }
case "$0" in /storage/*/Download/B6.sh) ;; *) fail_start INVALID_START_PATH ;; esac
B1_VOLUME=${0#/storage/}
B1_VOLUME=${B1_VOLUME%/Download/B6.sh}
case "$B1_VOLUME" in
...
parse_job() {
    ...
    case "$5" in PING|SNAPSHOT|STOP|CATALOG_BASELINE|CATALOG_READ|CATALOG_CLEANUP) ;; *) return 1 ;; esac
...
handle_catalog() {
    valid_sha "$B1_L5_EXPECTED_SHA" || { printf 'loader_error=EXPECTED_HASH_UNSET\n'; return 69; }
    read_snapshot "$B1_L5_PATH" 32768 || { printf 'loader_error=HELPER_READ_REJECTED\n'; return 65; }
    hash_text "$B1_TEXT" && [ "$B1_HASH" = "$B1_L5_EXPECTED_SHA" ] || {
        printf 'loader_error=HELPER_HASH_MISMATCH\n'; return 65;
    }
    case "$B1_OP" in
        CATALOG_BASELINE) B1_BUDGET=120 ;;
        CATALOG_READ) B1_BUDGET=210 ;;
        CATALOG_CLEANUP) B1_BUDGET=30 ;;
        *) return 65 ;;
    esac
    monotonic_seconds || return 70
    ...
    timeout "$B1_BUDGET" sh -c "$B1_TEXT" "$B1_L5_PATH" "$B1_OP" "$B1_SID"
}
...
dispatch() {
    case "$B1_OP" in
        ...
        CATALOG_BASELINE|CATALOG_READ|CATALOG_CLEANUP) handle_catalog ;;
...
worker() {
    ...
    B1_STARTED=$B1_NOW
    write_once "$B1_SESSION/session.receiver" "B1 RECEIVER $B1_SID B6 END$B1_LF" || { close_session ERROR; return 74; }
...
printf 'B6_START_REQUESTED sid=%s\n' "$B1_SID"
exit 0
