...
FINDUAS_OUTPUT_DIR="$FINDUAS_PROJECT_ROOT/build/policy-structure/armeabi-v7a"
FINDUAS_OUTPUT="$FINDUAS_OUTPUT_DIR/libfinduas_policy_structure.so"
...
"$FINDUAS_CLANG" -std=c11 -mthumb -shared -fPIC -fvisibility=hidden -Oz -Wall -Wextra -Werror \
  -I"$FINDUAS_JVMTI_HEADER_DIR" "$FINDUAS_PROJECT_ROOT/src/native/art_ti_policy_structure_probe.c" \
  "$FINDUAS_PROJECT_ROOT/src/native/payload_extract.c" \
  "$FINDUAS_PROJECT_ROOT/src/native/mediastore_sink.c" -llog -ldl -o "$FINDUAS_OUTPUT"
