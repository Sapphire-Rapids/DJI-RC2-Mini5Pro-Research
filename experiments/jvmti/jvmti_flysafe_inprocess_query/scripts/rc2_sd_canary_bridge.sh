#!/system/bin/sh
# B2: finite B1-protocol mailbox with one fixed ART TI canary experiment.
PATH=/system/bin
LC_ALL=C
export PATH LC_ALL
set -f
...
B1_L1_EXPECTED_SHA=1d889e7389c5561acbfca576dac9d9d60cf494c0d6fa5001e09a63cf153f7cc4
...
fail_start() { printf 'B2_ERROR code=%s\n' "$1"; exit 64; }
case "$0" in /storage/*/Download/B2.sh) ;; *) fail_start INVALID_START_PATH ;; esac
B1_VOLUME=${0#/storage/}
B1_VOLUME=${B1_VOLUME%/Download/B2.sh}
case "$B1_VOLUME" in
...
parse_job() {
    ...
    case "$5" in PING|SNAPSHOT|STOP|CANARY_BASELINE|CANARY_LOAD|CANARY_CLEANUP) ;; *) return 1 ;; esac
...
handle_canary() {
    valid_sha "$B1_L1_EXPECTED_SHA" || { printf 'loader_error=EXPECTED_HASH_UNSET\n'; return 69; }
    read_snapshot "$B1_L1_PATH" 32768 || { printf 'loader_error=HELPER_READ_REJECTED\n'; return 65; }
    hash_text "$B1_TEXT" && [ "$B1_HASH" = "$B1_L1_EXPECTED_SHA" ] || {
        printf 'loader_error=HELPER_HASH_MISMATCH\n'; return 65;
    }
    case "$B1_OP" in
        CANARY_BASELINE) B1_BUDGET=120 ;;
        CANARY_LOAD) B1_BUDGET=210 ;;
        CANARY_CLEANUP) B1_BUDGET=30 ;;
        *) return 65 ;;
    esac
    monotonic_seconds || return 70
    ...
    timeout "$B1_BUDGET" sh -c "$B1_TEXT" "$B1_L1_PATH" "$B1_OP" "$B1_SID"
}
...
dispatch() {
    case "$B1_OP" in
        ...
        CANARY_BASELINE|CANARY_LOAD|CANARY_CLEANUP) handle_canary ;;
...
worker() {
    ...
    B1_STARTED=$B1_NOW
    write_once "$B1_SESSION/session.receiver" "B1 RECEIVER $B1_SID B2 END$B1_LF" || { close_session ERROR; return 74; }
...
printf 'B2_START_REQUESTED sid=%s\n' "$B1_SID"
exit 0
