#!/system/bin/sh
# B4: finite B1-protocol mailbox with one fixed cloud-policy cache-read experiment.
PATH=/system/bin
LC_ALL=C
export PATH LC_ALL
set -f
...
B1_L3_EXPECTED_SHA=f9768d94ba3adaec7211da195f2743a67ca5213f2471f8e0a457224ce372b837
...
fail_start() { printf 'B4_ERROR code=%s\n' "$1"; exit 64; }
case "$0" in /storage/*/Download/B4.sh) ;; *) fail_start INVALID_START_PATH ;; esac
B1_VOLUME=${0#/storage/}
B1_VOLUME=${B1_VOLUME%/Download/B4.sh}
case "$B1_VOLUME" in
...
parse_job() {
    ...
    case "$5" in PING|SNAPSHOT|STOP|POLICY_BASELINE|POLICY_READ|POLICY_CLEANUP) ;; *) return 1 ;; esac
...
handle_policy() {
    valid_sha "$B1_L3_EXPECTED_SHA" || { printf 'loader_error=EXPECTED_HASH_UNSET\n'; return 69; }
    read_snapshot "$B1_L3_PATH" 32768 || { printf 'loader_error=HELPER_READ_REJECTED\n'; return 65; }
    hash_text "$B1_TEXT" && [ "$B1_HASH" = "$B1_L3_EXPECTED_SHA" ] || {
        printf 'loader_error=HELPER_HASH_MISMATCH\n'; return 65;
    }
    case "$B1_OP" in
        POLICY_BASELINE) B1_BUDGET=120 ;;
        POLICY_READ) B1_BUDGET=210 ;;
        POLICY_CLEANUP) B1_BUDGET=30 ;;
        *) return 65 ;;
    esac
    monotonic_seconds || return 70
    ...
    timeout "$B1_BUDGET" sh -c "$B1_TEXT" "$B1_L3_PATH" "$B1_OP" "$B1_SID"
}
...
dispatch() {
    case "$B1_OP" in
        ...
        POLICY_BASELINE|POLICY_READ|POLICY_CLEANUP) handle_policy ;;
...
worker() {
    ...
    B1_STARTED=$B1_NOW
    write_once "$B1_SESSION/session.receiver" "B1 RECEIVER $B1_SID B4 END$B1_LF" || { close_session ERROR; return 74; }
...
printf 'B4_START_REQUESTED sid=%s\n' "$B1_SID"
exit 0
