...
FINDUAS_BUILD="$FINDUAS_PROJECT_ROOT/build/cloud-cache-host-tests"
...
${CC:-cc} -std=c11 -Wall -Wextra -Werror -fsanitize=address,undefined -fno-omit-frame-pointer \
  ...
  "$FINDUAS_PROJECT_ROOT/tests/native/art_ti_cloud_cache_probe_test.c" "$FINDUAS_PROJECT_ROOT/src/native/cloud_policy_parser.c" -o "$FINDUAS_BUILD/test"
"$FINDUAS_BUILD/test"
