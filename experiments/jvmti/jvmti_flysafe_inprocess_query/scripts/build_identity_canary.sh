...
FINDUAS_OUTPUT_DIR="$FINDUAS_PROJECT_ROOT/build/identity-canary/armeabi-v7a"
FINDUAS_OUTPUT="$FINDUAS_OUTPUT_DIR/libfinduas_artti_identity.so"

test -x "$FINDUAS_CLANG"
test -f "$FINDUAS_JVMTI_HEADER_DIR/jvmti.h"
mkdir -p "$FINDUAS_OUTPUT_DIR"
"$FINDUAS_CLANG" \
  -std=c11 -shared -fPIC -fvisibility=hidden -O2 -Wall -Wextra -Werror \
  -I"$FINDUAS_JVMTI_HEADER_DIR" \
  "$FINDUAS_PROJECT_ROOT/src/native/art_ti_identity_canary.c" \
