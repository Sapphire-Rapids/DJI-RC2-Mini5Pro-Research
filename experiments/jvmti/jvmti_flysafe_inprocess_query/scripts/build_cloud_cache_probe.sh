...
FINDUAS_OUTPUT_DIR="$FINDUAS_PROJECT_ROOT/build/cloud-cache/armeabi-v7a"
FINDUAS_OUTPUT="$FINDUAS_OUTPUT_DIR/libfinduas_cloud_cache.so"
...
"$FINDUAS_CLANG" -std=c11 -mthumb -shared -fPIC -fvisibility=hidden -Oz -Wall -Wextra -Werror \
  -I"$FINDUAS_JVMTI_HEADER_DIR" "$FINDUAS_PROJECT_ROOT/src/native/art_ti_cloud_cache_probe.c" \
  "$FINDUAS_PROJECT_ROOT/src/native/cloud_policy_parser.c" -llog -ldl -o "$FINDUAS_OUTPUT"
file "$FINDUAS_OUTPUT"
