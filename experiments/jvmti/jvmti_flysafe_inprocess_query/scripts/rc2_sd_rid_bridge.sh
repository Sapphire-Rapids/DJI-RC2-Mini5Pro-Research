#!/system/bin/sh
# B3: finite B1-protocol mailbox with one fixed RID cache-read experiment.
PATH=/system/bin
LC_ALL=C
export PATH LC_ALL
set -f
...
B1_L2_EXPECTED_SHA=78ba8c4b2e7176048b16d582cd6c688f207131f4c246f0de45804ed47f478b52
...
fail_start() { printf 'B3_ERROR code=%s\n' "$1"; exit 64; }
case "$0" in /storage/*/Download/B3.sh) ;; *) fail_start INVALID_START_PATH ;; esac
B1_VOLUME=${0#/storage/}
B1_VOLUME=${B1_VOLUME%/Download/B3.sh}
case "$B1_VOLUME" in
...
parse_job() {
    ...
    case "$5" in PING|SNAPSHOT|STOP|RID_BASELINE|RID_READ|RID_CLEANUP) ;; *) return 1 ;; esac
...
handle_rid() {
    valid_sha "$B1_L2_EXPECTED_SHA" || { printf 'loader_error=EXPECTED_HASH_UNSET\n'; return 69; }
    read_snapshot "$B1_L2_PATH" 32768 || { printf 'loader_error=HELPER_READ_REJECTED\n'; return 65; }
    hash_text "$B1_TEXT" && [ "$B1_HASH" = "$B1_L2_EXPECTED_SHA" ] || {
        printf 'loader_error=HELPER_HASH_MISMATCH\n'; return 65;
    }
    case "$B1_OP" in
        RID_BASELINE) B1_BUDGET=120 ;;
        RID_READ) B1_BUDGET=210 ;;
        RID_CLEANUP) B1_BUDGET=30 ;;
        *) return 65 ;;
    esac
    monotonic_seconds || return 70
    ...
    timeout "$B1_BUDGET" sh -c "$B1_TEXT" "$B1_L2_PATH" "$B1_OP" "$B1_SID"
}
...
dispatch() {
    case "$B1_OP" in
        ...
        RID_BASELINE|RID_READ|RID_CLEANUP) handle_rid ;;
...
worker() {
    ...
    B1_STARTED=$B1_NOW
    write_once "$B1_SESSION/session.receiver" "B1 RECEIVER $B1_SID B3 END$B1_LF" || { close_session ERROR; return 74; }
...
printf 'B3_START_REQUESTED sid=%s\n' "$B1_SID"
exit 0
