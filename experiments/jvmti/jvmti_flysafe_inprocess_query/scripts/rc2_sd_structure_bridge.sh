#!/system/bin/sh
# B5: finite B1-protocol mailbox with one fixed matched-policy/DEFAULT private export.
PATH=/system/bin
LC_ALL=C
export PATH LC_ALL
set -f
...
B1_L4_EXPECTED_SHA=7baffaef8ea4167b443c8d5688c65cc6de7357bbf3663f66a30cd0c969b62713
...
fail_start() { printf 'B5_ERROR code=%s\n' "$1"; exit 64; }
case "$0" in /storage/*/Download/B5.sh) ;; *) fail_start INVALID_START_PATH ;; esac
B1_VOLUME=${0#/storage/}
B1_VOLUME=${B1_VOLUME%/Download/B5.sh}
case "$B1_VOLUME" in
...
parse_job() {
    ...
    case "$5" in PING|SNAPSHOT|STOP|STRUCTURE_BASELINE|STRUCTURE_READ|STRUCTURE_CLEANUP) ;; *) return 1 ;; esac
...
handle_structure() {
    valid_sha "$B1_L4_EXPECTED_SHA" || { printf 'loader_error=EXPECTED_HASH_UNSET\n'; return 69; }
    read_snapshot "$B1_L4_PATH" 32768 || { printf 'loader_error=HELPER_READ_REJECTED\n'; return 65; }
    hash_text "$B1_TEXT" && [ "$B1_HASH" = "$B1_L4_EXPECTED_SHA" ] || {
        printf 'loader_error=HELPER_HASH_MISMATCH\n'; return 65;
    }
    case "$B1_OP" in
        STRUCTURE_BASELINE) B1_BUDGET=120 ;;
        STRUCTURE_READ) B1_BUDGET=210 ;;
        STRUCTURE_CLEANUP) B1_BUDGET=30 ;;
        *) return 65 ;;
    esac
    monotonic_seconds || return 70
    [ "$((B1_NOW - B1_STARTED + B1_BUDGET + 10))" -lt 3600 ] || {
        printf 'loader_error=SESSION_EXPIRING\n'; return 124;
    }
    timeout "$B1_BUDGET" sh -c "$B1_TEXT" "$B1_L4_PATH" "$B1_OP" "$B1_SID"
}
...
dispatch() {
    case "$B1_OP" in
        ...
        STRUCTURE_BASELINE|STRUCTURE_READ|STRUCTURE_CLEANUP) handle_structure ;;
...
worker() {
    ...
    B1_STARTED=$B1_NOW
    write_once "$B1_SESSION/session.receiver" "B1 RECEIVER $B1_SID B5 END$B1_LF" || { close_session ERROR; return 74; }
...
printf 'B5_START_REQUESTED sid=%s\n' "$B1_SID"
exit 0
