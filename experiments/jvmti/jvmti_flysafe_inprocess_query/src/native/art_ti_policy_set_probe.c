...
#define CLOUD_TAG "FindUAS-Policy-Set"
#define CLOUD_SCHEMA "finduas-policy-set/v1"
...
/* Use the already initialized framework owner; no Application factory call. */
static jobject existing_application(JNIEnv *jni, jvmtiEnv *ti, jclass cls, int matches) {
    jint status = 0; jobject loader = NULL, thread = NULL, application = NULL;
    if (matches != 1 || cls == NULL ||
        (*ti)->GetClassStatus(ti, cls, &status) != JVMTI_ERROR_NONE ||
        !(status & JVMTI_CLASS_STATUS_INITIALIZED) || (status & JVMTI_CLASS_STATUS_ERROR) ||
        (*ti)->GetClassLoader(ti, cls, &loader) != JVMTI_ERROR_NONE) return NULL;
    if (loader != NULL) { (*jni)->DeleteLocalRef(jni, loader); return NULL; }
    jfieldID current = field(ti, cls, "sCurrentActivityThread", "Landroid/app/ActivityThread;", 1);
    jfieldID initial = field(ti, cls, "mInitialApplication", "Landroid/app/Application;", 0);
    if (!current || !initial) return NULL;
    thread = (*jni)->GetStaticObjectField(jni, cls, current);
    if (!(*jni)->ExceptionCheck(jni) && thread != NULL)
        application = (*jni)->GetObjectField(jni, thread, initial);
    if (thread != NULL) (*jni)->DeleteLocalRef(jni, thread);
    if ((*jni)->ExceptionCheck(jni)) {
        (*jni)->ExceptionClear(jni);
        if (application != NULL) (*jni)->DeleteLocalRef(jni, application);
        return NULL;
    }
    return application;
}
...
JNIEXPORT jint JNICALL Agent_OnAttach(JavaVM *vm, char *options, void *reserved) {
    ...
    jclass selected[7] = {0}; int matches[7] = {0};
    const char *wanted[7] = {
    ...
    jstring product_name = NULL;
    jbyteArray product_bytes = NULL;
    char *namespace_ascii = NULL, *policy_ascii = NULL;
    size_t namespace_length = 0, policy_length = 0;
    struct owner_guard sdk_guard = {0}; struct mmkv_guard mmkv_guard = {0};
    int export_rc = -1; size_t export_bytes = 0;
    struct global_payload_summary payload_summary = {
        .policy_rc = -1, .row_count = -1, .distinct_nonempty_count = -1,
        .nonempty_row_count = -1, .duplicate_country_row_count = -1,
        .default_row_count = -1, .first_default_present = -1,
        .first_default_nonempty = -1, .first_default_row_index = -1,
        .blocked_row_count = -1, .invalid_hex_row_index = -1, .json_length = 0
    };
    int stage = 1, exception = 0, product_count = 0, mmkv_count = 0;
    int namespace_present = -1, mmkv_present = -1, product_present = -1;
    int product_type = -1;
    ...
    for (jint i = 0; i < class_count; ++i) {
        ...
        for (int j = 0; j < 7; ++j) if (strcmp(signature, wanted[j]) == 0) { selected[j] = classes[i]; ++matches[j]; }
    ...
    stage = 9;
    char *payload_json = NULL; size_t payload_bytes = 0;
    int extract_rc = global_payload_extract(policy_ascii, policy_length, product_type,
        &payload_json, &payload_bytes, &payload_summary);
    json_rc = payload_summary.policy_rc;
    export_rc = 100 + extract_rc;
    if (json_rc != CLOUD_POLICY_OK && json_rc != CLOUD_POLICY_NAMESPACE_NULL &&
        json_rc != CLOUD_POLICY_PRODUCT_UNOBSERVED) { free(payload_json); goto done; }
    stage = 0;
    if (extract_rc == GLOBAL_PAYLOAD_OK) {
        jobject application = existing_application(jni, ti, selected[6], matches[6]);
        export_rc = 200;
        if (application != NULL) {
            struct mediastore_sink_result sink_result = {0};
            int sink_rc = mediastore_sink_write(jni, application, sid,
                (const unsigned char *)payload_json, payload_bytes, &sink_result);
            export_rc = sink_rc == 0 ? 0 : 300 + sink_rc;
            if (sink_rc == 0) export_bytes = payload_bytes;
            (*jni)->DeleteLocalRef(jni, application);
        }
    }
    free(payload_json);
    ...
    free(namespace_ascii); free(policy_ascii);
    if (jni != NULL && *jni != NULL) {
        exception |= clear_exception(jni);
        jobject refs[] = {loader,namespace_enum,namespace_text,root_dir,storage_key,stored,product_name,mmkv_wrapper,product_bytes};
    ...
    __android_log_print(ANDROID_LOG_INFO, CLOUD_TAG,
        "schema=" CLOUD_SCHEMA " phase=export sid=%s pid=%ld export_rc=%d export_bytes=%lu"
        " row_count=%d nonempty_count=%d distinct_count=%d default_present=%d default_nonempty=%d",
        sid,(long)pid,export_rc,(unsigned long)export_bytes,payload_summary.row_count,
        payload_summary.nonempty_row_count,payload_summary.distinct_nonempty_count,
        payload_summary.first_default_present,payload_summary.first_default_nonempty);
    __android_log_print(ANDROID_LOG_INFO, CLOUD_TAG,
        ...
        sid,(long)pid,(unsigned long)uid,(unsigned long)gid,(unsigned int)(sizeof(void *) * 8),
        ready,stage,exception,0,product_count,mmkv_count,
        namespace_present,mmkv_present,-1,product_present,product_type,-1,-1,
        json_rc,payload_summary.row_count,payload_summary.duplicate_country_row_count,-1,-1,-1,payload_summary.blocked_row_count,
