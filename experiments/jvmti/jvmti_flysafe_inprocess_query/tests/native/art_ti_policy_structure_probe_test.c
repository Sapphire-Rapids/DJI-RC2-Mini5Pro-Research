...
#include <stdarg.h>
#include <sys/types.h>
#include "../../src/native/art_ti_policy_structure_probe.c"
...
int mediastore_sink_write(JNIEnv *env, jobject context, const char sid[17],
    ...
    CHECK(strstr((const char*)json,"\"matched_hex\":\"AA\"") && !strstr((const char*)json,"country_code"));
    return fault==41?1:0;
}
static void run_case(int f,const char *name,int expected_stage,int c,int p,int m) {
    ...
    CHECK(Agent_OnAttach(&vm,sid,NULL)==JNI_OK);CHECK(log_count==3&&live_allocs==0&&pending==0&&!locked);CHECK(opened==closed);CHECK(cloud_calls==c&&product_calls==p&&decode_calls==m);CHECK(disposals==(f==1||f==2?0:1));
    ...
    if(f==0)CHECK(strstr(logs[1],"export_rc=0 "));
    if(f==40)CHECK(strstr(logs[1],"export_rc=200 "));
    if(f==41)CHECK(strstr(logs[1],"export_rc=301 "));
    CHECK(Agent_OnAttach(&vm,sid,NULL)==JNI_OK);CHECK(log_count==3&&cloud_calls==c&&product_calls==p&&decode_calls==m);
}
int main(void) {
    run_case(40,"missing Application",0,1,1,1);run_case(41,"storage error",0,1,1,1);
    ...
    test_name="invalid options";reset(0);char invalid[]="bad";CHECK(Agent_OnAttach(&vm,invalid,NULL)==JNI_ERR);CHECK(log_count==0&&cloud_calls==0);puts("Policy structure probe: 42 host cases passed");return 0;
}
