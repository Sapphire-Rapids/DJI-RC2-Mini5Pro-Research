...
#include <stdarg.h>
#include <sys/types.h>
#include "../../src/native/art_ti_policy_set_probe.c"
...
static unsigned char product[4];
static jsize product_size;
...
static const char *class_sigs[] = {
    ...
    "Luav/component/CloudControl/CloudControlNamespaces;", "Lcom/tencent/mmkv/MMKV;", "Landroid/app/ActivityThread;"
};
...
static jvmtiError JNICALL classes(jvmtiEnv *env,jint *count,jclass **out) {
    (void)env;if(fault==3)return JVMTI_ERROR_INTERNAL;*count=fault==4?8:7;*out=allocate(sizeof(jclass)*(size_t)*count);
    for(int i=0;i<*count;++i)(*out)[i]=(jclass)(uintptr_t)(i%7+1);return 0;
}
...
static jvmtiError JNICALL loader(jvmtiEnv *env,jclass cls,jobject *out) {
    (void)env;*out=cls==(jclass)7?NULL:(jobject)(uintptr_t)(fault==6?500+(uintptr_t)cls:500);return 0;
}
static jvmtiError JNICALL fields(jvmtiEnv *env,jclass cls,jint *count,jfieldID **out) {
    (void)env;uintptr_t n=(uintptr_t)cls;CHECK(n==2||n==4||n==5||n==6||n==7);*count=n==4?9:2;
    *out=allocate((size_t)*count*sizeof(jfieldID));
    for(int i=0;i<*count;++i)(*out)[i]=(jfieldID)(uintptr_t)(n==2?100+i:n==4?i+1:n==5?110+i:n==6?120+i:130+i);return 0;
}
static jvmtiError JNICALL field_name(jvmtiEnv *env,jclass cls,jfieldID id,char **name,char **sig,char **generic) {
    ...
    else if(x==130){*name=copy("sCurrentActivityThread");*sig=copy("Landroid/app/ActivityThread;");}
    else if(x==131){*name=copy("mInitialApplication");*sig=copy("Landroid/app/Application;");}
...
static jvmtiError JNICALL field_mods(jvmtiEnv *env,jclass cls,jfieldID id,jint *mods) {
    (void)env;(void)cls;uintptr_t n=(uintptr_t)id;*mods=(n==100||n==101||n==110||n==120||n==130)?ACC_STATIC:0;return 0;
}
...
static jobject JNICALL static_object(JNIEnv *env,jclass cls,jfieldID id) {
    (void)env;
    if(cls==(jclass)2){CHECK(id==(jfieldID)101);return fault==9?NULL:(jobject)610;}
    if(cls==(jclass)5){CHECK(id==(jfieldID)110);return fault==24?NULL:(jobject)700;}
    if(cls==(jclass)7){CHECK(id==(jfieldID)130);return fault==40?NULL:(jobject)710;}
...
static jobject JNICALL object_field(JNIEnv *env,jobject obj,jfieldID id) {
    ...
    if(obj==(jobject)710){CHECK(id==(jfieldID)131);return (jobject)711;}
...
static const char *string_value(jstring str) {
    if(str==(jstring)1001)return fault==13?"WrongProduct":"ProductType";
...
static jobject JNICALL invoke_static(JNIEnv *env,jclass cls,jmethodID id,const jvalue *args) {
    ...
    if(args[5].l==(jobject)1000){++cloud_calls;CHECK(0);}
    CHECK(args[5].l==(jobject)1001);CHECK(++product_calls==1);
    if(fault==15){pending=1;return NULL;}return fault==23?NULL:(jobject)707;
}
...
static jsize JNICALL array_length(JNIEnv *env,jarray a) { (void)env;CHECK(a==(jarray)707);return product_size; }
static void JNICALL byte_region(JNIEnv *env,jbyteArray a,jsize start,jsize len,jbyte *out) {
    (void)env;CHECK(!locked);CHECK(a==(jbyteArray)707);CHECK(start>=0&&len>=0&&start+len<=product_size);memcpy(out,product+start,(size_t)len);if(fault==19)pending=1;
}
static int native_owner_open(struct owner_guard *guard) { guard->handle=(void*)2;return fault!=14; }
static int native_owner_same(const struct owner_guard *guard) { CHECK(guard->handle&&!locked);++same_calls;return fault!=21&&!(fault==18&&same_calls==2); }
...
static void reset(int f) {
    ...
    product[0]=139;product[1]=product[2]=product[3]=0;product_size=4;
...
int mediastore_sink_write(JNIEnv *env, jobject context, const char sid[17],
    const unsigned char *json, size_t bytes, struct mediastore_sink_result *result) {
    (void)result; CHECK(env==&jni_env && context==(jobject)711 && !locked && pending==0);
    CHECK(strcmp(sid,"0123456789abcdef")==0 && bytes>0 && bytes<=32768);
    CHECK(strstr((const char*)json,"\"data_hex\":\"AA\"") && strstr((const char*)json,"\"country_code\":\"DEFAULT\"") && strstr((const char*)json,"\"blocked_for_product\":true"));
    CHECK(!strstr((const char*)json,"TEST_RID_NAMESPACE")&&!strstr((const char*)json,"current_area")&&!strstr((const char*)json,"matched_hex"));
    return fault==41?1:0;
}
static void run_case(int f,const char *name,int expected_stage,int p,int m) {
    test_name=name;reset(f);char sid[]="0123456789abcdef";
    if(f==16){for(int i=0;i+1<policy_units;++i)if(policy[i]=='A'&&policy[i+1]=='A'){policy[i+1]='Z';break;}}
    if(f==17)product[2]=1;if(f==22)product_size=3;if(f==36)policy[0]='[';if(f==38)policy[1]=0xdc00;
    ...
    CHECK(Agent_OnAttach(&vm,sid,NULL)==JNI_OK);CHECK(log_count==3&&live_allocs==0&&pending==0&&!locked);CHECK(opened==closed);CHECK(cloud_calls==0&&product_calls==p&&decode_calls==m);CHECK(disposals==(f==1||f==2?0:1));
    const int expected_tokens[3]={7,11,32};
    for(int i=0;i<3;++i){int tokens=1;for(const char *s=logs[i];*s;++s)if(*s==' ')++tokens;CHECK(tokens==expected_tokens[i]);CHECK(strncmp(logs[i],"schema=finduas-policy-set/v1 ",28)==0);}
    char target[40];snprintf(target,sizeof(target)," stage=%d ",expected_stage);CHECK(strstr(logs[2],target));CHECK(strstr(logs[2],expected_stage?" ready=0 ":" ready=1 "));
    if(f==0||f==37)CHECK(strstr(logs[2],"json_rc=0 entry_count=2 duplicate_count=0 candidate_count=-1 match_count=-1 default_match=-1 product_blocked_count=1"));
    CHECK(!strstr(logs[2],"TEST_RID")&&!strstr(logs[2],"DEFAULT")&&!strstr(logs[2],"AA")&&!strstr(logs[2],"country"));
    CHECK(strstr(logs[2],"cloud_query_count=0 ")&&strstr(logs[2],"cloud_present=-1 ")&&strstr(logs[2],"receiver_type=-1 receiver_index=-1 "));
    if(f==0||f==37){CHECK(strstr(logs[1],"export_rc=0 "));CHECK(strstr(logs[1],"row_count=2 nonempty_count=2 distinct_count=2 default_present=1 default_nonempty=1"));}
    if(f==16)CHECK(strstr(logs[1],"export_rc=106 "));
    if(f==40)CHECK(strstr(logs[1],"export_rc=200 "));
    if(f==41)CHECK(strstr(logs[1],"export_rc=301 "));
    CHECK(Agent_OnAttach(&vm,sid,NULL)==JNI_OK);CHECK(log_count==3&&cloud_calls==0&&product_calls==p&&decode_calls==m);
}
int main(void) {
    run_case(40,"missing Application",0,1,1);run_case(41,"storage error",0,1,1);
    run_case(0,"global policy set",0,1,1);run_case(1,"JNI unavailable",1,0,0);run_case(2,"ART TI unavailable",2,0,0);
    run_case(3,"enumeration failure",3,0,0);run_case(4,"duplicate class",4,0,0);run_case(5,"uninitialized",4,0,0);run_case(6,"loader mismatch",4,0,0);
    run_case(7,"missing field",5,0,0);run_case(8,"method mismatch",5,0,0);run_case(9,"null key",5,0,0);run_case(10,"key class",5,0,0);run_case(11,"component",5,0,0);run_case(12,"key flags",5,0,0);run_case(13,"key name",5,0,0);
    run_case(14,"SDK owner unavailable",10,0,0);run_case(15,"product cache exception",8,1,1);run_case(16,"invalid payload hex",0,1,1);run_case(17,"product out of range",8,1,1);run_case(18,"owner changed after query",10,1,1);run_case(19,"array exception",8,1,1);run_case(20,"dispose error",13,1,1);run_case(21,"owner changed before query",10,0,1);run_case(22,"bad product length",8,1,1);run_case(23,"product absent",0,1,1);
    run_case(24,"namespace absent",6,0,0);run_case(25,"MMKV not initialized",6,0,0);run_case(26,"MMKV library absent",7,0,0);run_case(27,"global busy",7,0,0);run_case(28,"instance absent",7,0,0);run_case(29,"instance busy",7,0,0);run_case(30,"instance reload",7,0,0);run_case(31,"multi process",7,0,0);run_case(32,"wrapper handle mismatch",8,0,0);run_case(33,"MMKV value size",8,0,0);run_case(34,"MMKV decode exception",8,0,1);run_case(35,"stored namespace absent",0,1,1);run_case(36,"malformed JSON",9,1,1);run_case(37,"UTF16 to UTF8",0,1,1);run_case(38,"invalid UTF16",9,0,1);run_case(39,"registry read",7,0,0);
    test_name="invalid options";reset(0);char invalid[]="bad";CHECK(Agent_OnAttach(&vm,invalid,NULL)==JNI_ERR);CHECK(log_count==0&&cloud_calls==0);puts("Policy set probe: 43 host cases passed");return 0;
}
