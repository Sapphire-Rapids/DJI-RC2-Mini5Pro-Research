...
#error "FindUAS EID route resolver V2.2 must be built for AArch64"
#endif

#define FINDUAS_LOG_TAG "FindUAS-EID-Route-V22"
...
static void emit_numeric_result(const FinduasAgentResult *result) {
    __android_log_print(
        ANDROID_LOG_INFO,
        FINDUAS_LOG_TAG,
        "FINDUAS_EID_ROUTE_V22 error=%u route=%u loaded=%d on=%d gate=%d loaders=%d "
