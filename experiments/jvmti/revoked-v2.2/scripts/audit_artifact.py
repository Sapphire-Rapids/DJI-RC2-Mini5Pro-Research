#!/usr/bin/env python3
"""Fail-closed artifact/source audit for the offline EID route resolver V2.2."""
...
EXPECTED_PACKAGE = "com.finduas.jvmti.eidroute.v22"
EXPECTED_NATIVE_ENTRY = "lib/arm64-v8a/libfinduas_eid_route_resolver_v2_2.so"
...
def configure_java_runtime() -> None:
    for candidate in (
        os.environ.get("FINDUAS_ROUTE_V22_JAVA_HOME"),
...
def audit_compiled_exception_gate(elf: ExactElf) -> None:
    ...
    gate_rva = 0x1240
    gate_check_rva = 0x8218
    dormant_helper_rva = 0x822C
    gate_call_rva = 0x8070
    gate_true_branch_rva = 0x808C
    false_exit_branch_rva = 0x80B4
    dormant_call_rva = 0x80C0
    ...
    gate_control_block = bytes.fromhex(
        ...
        "e8079f1a"  # cset w8, ne
        "e91f40f9"  # ldr x9, [sp, #0x38]
        "281900b9"  # str w8, [x9, #0x18]
        "e81f40f9"  # ldr x8, [sp, #0x38]
        "081940b9"  # ldr w8, [x8, #0x18]
        "68010035"  # cbnz w8, admitted block
        "01000014"  # b false block
        "e91f40f9"  # false: ldr x9, [sp, #0x38]
        "a8008052"  # mov w8, #5 (EXCEPTION_BOUNDARY_UNPROVEN)
        "280100b9"  # str w8, [x9]
        "e0030791"  # add x0, sp, #0x1c0
        "38fbff97"  # bl close module handles
        "e81f40f9"  # ldr x8, [sp, #0x38]
        "080140b9"  # ldr w8, [x8]
        "e84700b9"  # str w8, [sp, #0x44]
...
def audit_compiled_identity_dominance(elf: ExactElf) -> None:
    """Bind the exact identity-success edge to symbol resolution and the later hard gate."""
    identity_block_rva = 0x7EB0
    identity_call_rva = 0x7EB8
    identity_target_rva = 0x8EF8
    identity_branch_rva = 0x7F34
    failure_status_rva = 0x7F40
    failure_close_call_rva = 0x7F4C
    failure_exit_rva = 0x7F5C
    finalizer_block_rva = 0x7F60
    finalizer_call_rva = 0x7F64
    finalizer_target_rva = 0x66D4
    finalizer_branch_rva = 0x7F7C
    finalizer_failure_status_rva = 0x7F88
    finalizer_close_call_rva = 0x7F94
    finalizer_failure_exit_rva = 0x7FA4
    resolver_call_rva = 0x7FB4
    resolver_target_rva = 0x8128
    exception_gate_call_rva = 0x8070
    dormant_call_rva = 0x80C0

    identity_block = bytes.fromhex(
        "e10740f9e003079110040094e81340f9e03300b9e9f340b9ea1f40f9491d00b9"
        "e9f740b9ea1f40f9492100b9e9fb40b9ea1f40f9492500b9e9ff40b9ea1f40f9"
        "492900b9082540f9e91f40f9281900f9e80b41b9e91f40f9283900b9e80f41b9"
        "e91f40f9283d00b9e81341b9e91f40f9284100b9e81741b9e91f40f9284500b9"
        "e83340b96801003401000014e91f40f908028052280100b9e00307918efbff97"
        "e81f40f9080140b9e84700b964000014"
    )
    require(
        elf.bytes_at(identity_block_rva, len(identity_block)) == identity_block,
        "identity result/fail-close block mismatch",
    )
    require(elf.data.count(identity_block) == 1, "identity dominance block cardinality")

    identity_call = struct.unpack("<I", elf.bytes_at(identity_call_rva, 4))[0]
    require(
        decode_aarch64_bl_target(identity_call_rva, identity_call) == identity_target_rva,
        "identity verifier call target mismatch",
    )
    require(
        struct.unpack("<I", elf.bytes_at(identity_branch_rva, 4))[0] == 0x34000168,
        "identity success CBZ edge mismatch",
    )
    require(
        struct.unpack("<I", elf.bytes_at(failure_status_rva, 4))[0] == 0x52800208,
        "identity failure status is not RUNTIME_IDENTITY_FAILED",
    )
    close_call = struct.unpack("<I", elf.bytes_at(failure_close_call_rva, 4))[0]
    require(
        decode_aarch64_bl_target(failure_close_call_rva, close_call) == 0x6D84,
        "identity failure does not close pinned target handles",
    )
    require(
        struct.unpack("<I", elf.bytes_at(failure_exit_rva, 4))[0] == 0x14000064,
        "identity failure does not skip symbol/target route",
    )

    finalizer_block = bytes.fromhex(
        "e0030791dcf9ff97e03700b9e83740b9e91f40f9280500b9e83740b968010034"
        "01000014e91f40f928008052280100b9e00307917cfbff97e81f40f9080140b9"
        "e84700b952000014e21f40f9e0030791e16304915d000094"
    )
    require(
        elf.bytes_at(finalizer_block_rva, len(finalizer_block)) == finalizer_block,
        "post-identity finalizer/fail-close/symbol-resolution block mismatch",
    )
    require(elf.data.count(finalizer_block) == 1, "post-identity finalizer block cardinality")
    finalizer_call = struct.unpack("<I", elf.bytes_at(finalizer_call_rva, 4))[0]
    resolver_call = struct.unpack("<I", elf.bytes_at(resolver_call_rva, 4))[0]
    require(
        decode_aarch64_bl_target(finalizer_call_rva, finalizer_call) == finalizer_target_rva,
        "post-identity module finalizer call target mismatch",
    )
    require(
        decode_aarch64_bl_target(resolver_call_rva, resolver_call) == resolver_target_rva,
        "symbol resolver call target mismatch",
    )
    require(
        struct.unpack("<I", elf.bytes_at(finalizer_branch_rva, 4))[0] == 0x34000168,
        "module finalizer success CBZ edge mismatch",
    )
    require(
        struct.unpack("<I", elf.bytes_at(finalizer_failure_status_rva, 4))[0] == 0x52800028,
        "module finalizer failure status mismatch",
    )
    finalizer_close_call = struct.unpack("<I", elf.bytes_at(finalizer_close_call_rva, 4))[0]
    require(
        decode_aarch64_bl_target(finalizer_close_call_rva, finalizer_close_call) == 0x6D84,
        "module finalizer failure does not close target handles",
    )
    require(
        struct.unpack("<I", elf.bytes_at(finalizer_failure_exit_rva, 4))[0] == 0x14000052,
        "module finalizer failure does not skip symbol/target route",
    )
    require(
        identity_call_rva < identity_branch_rva < finalizer_call_rva < resolver_call_rva <
        exception_gate_call_rva < dormant_call_rva,
        "identity/symbol/exception/dormant call order mismatch",
    )
...
def audit_source(project_dir: Path) -> None:
    ...
    require(
        source_names == [
            "abi_shims.S", "abi_shims.h", "agent.c", "elf_profile.h", "identity_core.c",
    ...
    identity_index = route.index("finduas_runtime_identity_verify(&modules, &identity)", run_marker)
    ...
    require(identity_index < finalize_index < symbol_index < gate_index < admitted_call_index, "identity/finalize/exception gate ordering mismatch")
    ...
    require("finduas_modules_recheck(modules)" in identity, "post-identity linker recheck absent")
...
def audit_manifest(aapt: Path, apk: Path) -> None:
    ...
    for label, pattern in {
        ...
        "fixed label": r'android:label[^\n]*="FindUAS EID route resolver V2\.2 offline carrier"',
...
def audit_elf(ndk_bin: Path, library_path: Path) -> None:
    ...
    require("Library soname: [libfinduas_eid_route_resolver_v2_2.so]" in dynamic, "unexpected SONAME")
    ...
    for required in (
        "FindUAS-EID-Route-V22",
        "FINDUAS_EID_ROUTE_V22 error=",
        "libsdk_jni.so",
        "libsdk_key_value.so",
        "libsdk_base.so",
        "EIDSwitch",
...
def audit_apk(apk: Path, project_dir: Path, sdk_root: Path, ndk_root: Path) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="finduas-route-v22-audit-") as temporary:
        library_path = Path(temporary) / "libfinduas_eid_route_resolver_v2_2.so"
...
def main() -> int:
    ...
    ndk_root = Path(os.environ.get("FINDUAS_ROUTE_V22_NDK_ROOT", str(sdk_root / "ndk/27.2.12479018"))).resolve()
