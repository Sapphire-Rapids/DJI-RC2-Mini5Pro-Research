...
ndk_root=${FINDUAS_ROUTE_V22_NDK_ROOT:-$sdk_root/ndk/27.2.12479018}
build_tools_root=${FINDUAS_ROUTE_V22_BUILD_TOOLS_ROOT:-$sdk_root/build-tools/35.0.0}
jdk_root=${FINDUAS_ROUTE_V22_JAVA_HOME:-${JAVA_HOME:-}}
: "${jdk_root:?Set FINDUAS_ROUTE_V22_JAVA_HOME or JAVA_HOME}"
gradle_bin=${FINDUAS_ROUTE_V22_GRADLE_BIN:-$(command -v gradle || true)}
: "${gradle_bin:?Set FINDUAS_ROUTE_V22_GRADLE_BIN or put gradle on PATH}"
debug_keystore=${FINDUAS_ROUTE_V22_KEYSTORE:-$HOME/.android/debug.keystore}
keystore_alias=${FINDUAS_ROUTE_V22_KEY_ALIAS:-androiddebugkey}
keystore_password=${FINDUAS_ROUTE_V22_KEYSTORE_PASSWORD:-android}
key_password=${FINDUAS_ROUTE_V22_KEY_PASSWORD:-android}
...
unsigned_apk=$package_stage/route-v22-no-dex-unsigned.apk
aligned_apk=$package_stage/route-v22-no-dex-aligned.apk
final_apk=$dist_dir/FindUAS-JVMTI-EID-Route-Resolver-V2.2-0.1.0-offline-unadmitted-arm64-v8a.apk
native_entry=lib/arm64-v8a/libfinduas_eid_route_resolver_v2_2.so
