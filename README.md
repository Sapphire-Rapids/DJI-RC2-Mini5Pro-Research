...
- **当前实机进展（C-277--C-282）**：A-051 在原 Fly 进程完成一次官方同步缓存读取，
  返回 RID 支持/正常1/1、EID0/0、失败原因0。JNI/解析/环境释放成功，PID/UID/APK 稳定；
  测试文件删除并独立确认，B3 正常停止。无需 root。下一步对齐独立 RID 接收/电机时序；
  详见[实机结果](docs/23_RC2_LIVE_RUNTIME.md#实机-rid-缓存结果)。
